...
\item The functions \(\{S_N\}_{N=0}^\infty\) are uniformly bounded, i.e., there exists \(C_0>1\)  such that \(|S_N|\leq C_0\) for all \(N\in\mathbb N_0\).
...
\ee
which converges a.e. to \(0\) by our assumption (\textbf{A1}). So for fixed \(t\), \(d_N(t,s)\) converges a.e. to \(0\) for \(s\in[0,t]\).Furthermore, we can uniformly bound \(d_N(t, s)\) by our assumption \textbf{(A2)}.Thus by the Bounded Convergence Theorem, we have \( \int^t_0 d_N (t, s) \d s \to 0\) as \(N \to \infty.\)
